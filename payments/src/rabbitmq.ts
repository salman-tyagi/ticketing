...
export async function connectRabbitMQ() {
  while (!shuttingDown) {
    try {
      ...
      await channel.assertQueue(Queues.PaymentsServiceOrderCreated, {
        durable: true,
      });

      await channel.assertQueue(Queues.PaymentsServiceOrderCancelled, {
        durable: true,
      });

      await channel.bindQueue(Queues.PaymentsServiceOrderCreated, Exchanges.Order, RoutingKeys.OrderCreated);
      await channel.bindQueue(Queues.PaymentsServiceOrderCancelled, Exchanges.Order, RoutingKeys.OrderCancelled);
