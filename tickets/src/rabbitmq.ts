...
export async function connectRabbitMQ() {
  while (!shuttingDown) {
    try {
      ...
      await channel.assertQueue(Queues.TicketsServiceOrderCreated, {
        durable: true,
      });

      await channel.assertQueue(Queues.TicketsServiceOrderCancelled, {
        durable: true,
      });

      await channel.bindQueue(Queues.TicketsServiceOrderCreated, Exchanges.Order, RoutingKeys.OrderCreated);
      await channel.bindQueue(Queues.TicketsServiceOrderCancelled, Exchanges.Order, RoutingKeys.OrderCancelled);
